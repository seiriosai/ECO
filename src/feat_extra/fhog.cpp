...
void gradMag( float *I, float *M, float *O, int h, int w, int d, bool full ) {
  ...
  for( x=0; x<w; x++ ) {
    ...
    for( y=0; y<h4/4; y++ ) {
      _m = sse::_MIN_( sse::RCPSQRT(_M2[y]), sse::SET(1e10f) );
      _M2[y] = sse::RCP(_m);
...
shared_ptr<eco::Feature> fhog(const cv::Mat& img, int binSize, int nOrients, float clip, bool crop){
    ...
    if(channel == 3)
	{
      for(int i=0;i<width;++i)
          ...
          {
              data = img.data + j*width*3 + i*3;
              IMG[0] = data[0];
              IMG[height*width] = data[1];
              IMG[height*width*2] = data[2];
              IMG++;
          }
    }
    else
